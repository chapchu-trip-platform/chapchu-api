...
class PostRecommendationControllerTest {
  ...
  void 게시글을_추천한다() throws Exception {
    ...
    mockMvc
        .perform(
            post("/posts/{postId}/recommendations", postId)
    ...
    verify(postService).recommend(USER_ID, postId);
  }
  ...
  void 게시글_추천을_취소한다() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/posts/{postId}/recommendations", postId)
                .with(jwt().jwt(j -> j.subject(USER_ID.toString()))))
    ...
    verify(postService).cancelRecommendation(USER_ID, postId);
  }
}
