...
## 12. 외부 키 및 보안 설정 주입 규칙
- **API 키, OAuth 시크릿, DB 비밀번호 등 모든 보안 설정값은 코드나 설정 파일에 하드코딩하지 마라.**
- 환경변수 플레이스홀더(`${TOUR_API_KEY}`, `${AWS_ACCESS_KEY}` 등)만 `application.yml`에 선언하고, 실제 값은 개발자가 직접 주입한다.
- 새 외부 서비스 연동 시 필요한 환경변수 목록만 `application.yml`에 추가하고, 값 주입은 개발자에게 맡겨라.
- 테스트용 더미값이 필요하면 `app/src/test/resources/application.yml`에만 추가하라.
